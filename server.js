...
app.post('/api/oracle', async (req, res) => {
    try {
        ...
        if (voiceEnabled) {
            const voiceResponse = await fetch(`https://generativelanguage.googleapis.com/v1alpha/models/gemini-2.0-flash-exp:generateContent?key=${GEMINI_API_KEY}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (voiceResponse.ok) {
                ...
                // Also pass the error back so the client knows exactly why it failed
                wisdom += ` [Voice Generation Failed: ${voiceResponse.status}]`;
            }
        }
